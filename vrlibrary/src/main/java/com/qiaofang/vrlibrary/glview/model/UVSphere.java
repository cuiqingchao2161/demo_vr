package com.qiaofang.vrlibrary.glview.model;
...
public class UVSphere {
    ...
    public UVSphere(float radius, int divide, int eastSide) {
        this();
    ...
    private void makeSphereVertices(float radius, int divide, int eastSide) {
        float altitude = 0.0f;
        ...
        if (eastSide == 0) {
            ...
            startPoint = Math.PI*3/2;
        }

        for(int i = 0; i < divide/4; ++i)
        {
            altitude      = (float) (Math.PI/4.0 -    i    * (Math.PI*2) / divide);
            altitudeDelta = (float) (Math.PI/4.0 - (i + 1) * (Math.PI*2) / divide);
