...
public class GLPhotoView extends GLSurfaceView {
    ...
    public static float mAngleZ = 0;// 摄像机所在的z坐标
...
//            distanceX = 0.1f * (-distanceX) / activity.getWindowManager().getDefaultDisplay().getWidth();
    ...
    public class MyGLRender implements GLSurfaceView.Renderer {
        ...
        public MyGLRender() {
            ...
            int index = 0;
            int index1 = 0;
            double d = CAP * Math.PI / 180;//每次递增的弧度,算法解释：一共递增180/CAP次，半周弧度一共是PI,所以每次递增PI除以（180/CAP)结果就是CAP*PI/180
        ...
        public void onSurfaceChanged(GL10 gl, int width, int height) {
            ...
            try {
                bitmap = BitmapFactory.decodeStream(QfAdTvApp.getApp().getApplicationContext().getResources().getAssets().open("R0010158.jpg"));
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//            Matrix.frustumM(mProjectionMatrix, 0, -1f, 1f, -ratio, ratio, 3, 10);

            //控制相机位置 1无畸变  其他播放图像边缘有畸变
            float currentDegree= (float) (Math.toDegrees(Math.atan(1)) * 0.6);//
            Matrix.perspectiveM(mProjectionMatrix, 0, currentDegree, ratio,3f, 200f);
        }
        ...
        public void onDrawFrame(GL10 gl) {
            ...
            Matrix.setLookAtM(mCameraMatrix, 0, mCameraPosX, mCameraPosY, mCameraPosZ, mCameraDirectionX, mCameraDirectionZ, mCameraDirectionY, 0, 1, 0);
            Matrix.perspectiveM(mProjectionMatrix, 0, 100, 1920/840,3f, 200f);
...
//            GLES20.glUniformMatrix4fv(mViewMatrixHandle, 1, false, mCameraMatrix, 0);
